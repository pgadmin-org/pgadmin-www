...
<p>' . _("Once installed, you will need to create a <i>config_local.py</i> file in the same directory as <i>config.py</i>. On a machine with a virtual environment created at <i>~/pgadmin4</i>, this is <i>lib/python2.7/site-packages/pgadmin4</i>. Ensure you set values for the SECRET_KEY, SECURITY_PASSWORD_SALT and CSRF_SESSION_KEY settings at bare minimum - see config.py for more information and other settings that can be customised. In order to reference other variables from <i>config.py</i>, you may need to include <code>from config import *</code> at the top of <config_local.py</p>") . '</p>

<p>' . _("pgAdmin can now be run with a command like <code>python /path/to/virtualenv/lib/python2.7/site-packages/pgadmin4/pgAdmin4.py</code>. Finally, point your browser to <i>http://127.0.0.1:5050</i>.") . '</p>
